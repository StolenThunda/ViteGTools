        ...
        <q-card-section>
          <q-list dense>
            ...
            >
              ...
              <q-item-label class="text-h5 text-weight-bolder">Selections</q-item-label>
            </q-item>
            <q-item
...
export default defineComponent( {
  ...
  data: () => ( {
    ...
    SHAPEFILL: { color: "#777777", opacity: 0.3 },
    SHAPESTROKE: { color: "#aaaaaa", opacity: 2 },
    CHORDI: "#0099ff",
    CHORDIV: "#FFEF00",
    CHORDV: "#FF0000",
    MAJOR: "#3bc33b",
  ...
  mounted () {
    ...
    this.Body.stroke( { width: 3, color: "#555" } );

    this.Headstock = this.Drawing.path(
      "M1.0,50.5 C-65.5,45.2 -111.3,36.9 -136.2, 25.6 C-173.6, 8.6 -209.1, -25.5 -223.0, -52.0 C-232.4, -69.7 -240.3, -81.3 -247.0,-87.0 L -246.0,326.2 L -147.2, 356.2 C-134.6, 361.3 -121.4, 361.3 -107.8, 356.2 C-87.3, 348.7 -68.4, 312.4 -44.4, 291.7 C-28.4, 277.9 -13.3, 270.0 1.0, 268.0 L 1.0, 50.5 Z"
    );
    this.Headstock.fill( this.HEADSTOCKFILL );
    this.Headstock.stroke( { width: 3, color: "#555" } );

    this.Fretboard = this.Drawing.path(
      "M3,51.955	L2455.262,3	C2468.431,4.1024	2478.30815,7.96089385	2484.89264,14.575419	C2494.76938,24.4972067	2503,107.178771	2503,151	C2503,194.821229	2494.76938,275.849162	2484.89264,287.424581	C2478.30815,295.141527	2468.43141,299	2455.26243,299	L3.64612326,266.581006	L3,51.9553073	Z"
    );
    this.Fretboard.fill( this.FRETBOARDFILL );
    this.Fretboard.stroke( { width: 3, color: "#444" } );
  ...
  */
  ...
  computed: {
    ...
    selectionsAvailable () {
      return (
        ...
        this.accScales.length +
        this.accRootNotes.length >
        0
      );
    },
    scalesSelected () {
  ...
  methods: {
    ...
    setSelectionDialog ( v ) {
      if ( !v ) {
        this.selectionDialog = true;
      }
      this.selectionDialog = v
    },
    setKey ( k ) { this.key = k; this.refreshUI(); },
    drawerClick ( e ) {
    ...
    updateFretboard () {
      ...
      console.log( 'Fretboard refreshed' )
    },
    createDots () {
      this.allDots = this.Drawing.group();
      this.Dots.push( { x: 502.5, y: 160.5 } );
      this.Dots.push( { x: 820.5, y: 158.5 } );
      this.Dots.push( { x: 1103.5, y: 158.5 } );
      this.Dots.push( { x: 1359.5, y: 157.5 } );
      this.Dots.push( { x: 1689.5, y: 97.5 } );
      this.Dots.push( { x: 1969.5, y: 155.5 } );
      this.Dots.push( { x: 2131.5, y: 155.5 } );
      this.Dots.push( { x: 2274.5, y: 153.5 } );
      this.Dots.push( { x: 2402.5, y: 153.5 } );
      this.Dots.push( { x: 1689.5, y: 214.5 } );

      this.SideDots.push( { x: 502.5, y: 300 } );
      this.SideDots.push( { x: 820.5, y: 303 } );
      this.SideDots.push( { x: 1103.5, y: 305 } );
      this.SideDots.push( { x: 1359.5, y: 310 } );
      this.SideDots.push( { x: 1670, y: 314 } );
      this.SideDots.push( { x: 1969.5, y: 320 } );
      this.SideDots.push( { x: 2131.5, y: 322 } );
      this.SideDots.push( { x: 2274.5, y: 325 } );
      this.SideDots.push( { x: 2402.5, y: 328 } );
      this.SideDots.push( { x: 1700, y: 315 } );

      for ( let i = 0; i < this.Dots.length; i++ ) {
        this.allDots
          .circle( 30 )
          .move( this.Dots[i].x - 15, this.Dots[i].y - 15 )
          .fill( "#333" );
      }
      ...
      for ( let i = 0; i < this.SideDots.length; i++ ) {
        this.allDots
          .circle( 10 )
          .move( this.SideDots[i].x - 5, this.SideDots[i].y - 5 )
          .fill( "#FFF" );
      }
    },
    createStrings () {
      ...
      this.Strings.push(
        { pt1: { x: 0, y: 67.5 }, pt2: { x: 2489, y: 26 } },
      ...
      for ( let i = 0; i < this.Strings.length; i++ ) {
        var stringString =
          ...
          " ";
        stringString +=
          this.Strings[i].pt2.x +
          "," +
          ( this.Strings[i].pt2.y - stringWidths[i] / 2.0 ) +
          " ";
        stringString +=
          this.Strings[i].pt2.x +
          "," +
    ...
    computeOffsets () {
      for ( let i = 0; i < this.Frets.length; i++ ) {
        ...
        if ( i < this.Frets.length - 1 ) {
          this.xoffsets.push(
            ( this.Frets[i + 1].pt1.x - this.Frets[i].pt1.x ) / 4.0
          );
        }
      }
    },
    ...
    createFretMarkers () {
      ...
      for ( let i = 1; i < theMarkers.length; i++ ) {
        var polygonString =
          theMarkers[i - 1].pt1.x +
          markerOffset +
          "," +
          ( theMarkers[i - 1].pt1.y - markerVOffset ) +
          " ";
        polygonString +=
          theMarkers[i - 1].pt2.x +
          markerOffset +
          "," +
          ( theMarkers[i - 1].pt2.y + markerVOffset ) +
          " ";
        polygonString +=
          theMarkers[i].pt2.x -
          markerOffset +
          "," +
          ( theMarkers[i].pt2.y + markerVOffset ) +
          " ";
        polygonString +=
          theMarkers[i].pt1.x -
          markerOffset +
          "," +
          ( theMarkers[i].pt1.y - markerVOffset );
