...
function setMessage ( strMessage ) { $( '#message' ).html( strMessage ); }
...
function updateProgress () {
  ...
  if ( nTotalCount > 0 ) {
    ...
    stringPercentage = ( nStringIndex - 1 ) * ( ( 100 / nTotalCount ) * 0.1 );
...
function playClick ( tickOrTock ) {
  if ( tickOrTock == 0 ) { tickSound.play(); }
  else { tockSound.play(); }
}
...
function toggleSidebar () { $( '#pageWrapper' ).toggleClass( 'split' ); }
        ...
        if ( nCombinationIndex == selectedCombinations.length - 1 ) {
          ...
          if ( !bRestFinished ) {
            ...
            if ( nRestCycle == 0 ) {
              ...
              let restString = "Rest... " + ( nRestCycle + 1 );
              restString += "<br/><small>Next: " + getformattedCombination( nCombinationIndex + 1 ) + "</small>";
              setMessage( restString );
